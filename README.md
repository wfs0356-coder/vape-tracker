...
- 액상 이름을 입력하면 자동 저장됩니다.
- 액상을 교체한 날 **오늘로 설정**을 누르면 그날 날짜로 기록됩니다.
- **교체 경고 기준은 30일로 고정**되어 있습니다. 25일째부터 노란색 "곧 교체", 30일이 지나면 빨간색 "교체 필요"로 표시됩니다. (기준일을 바꾸고 싶으면 언제든 요청하세요 — 코드 상수 하나만 바꾸면 됩니다.)
- 삭제는 실수 방지를 위해 ✕ 버튼을 두 번 눌러야 합니다.
- 다른 직원이 다른 기기에서 값을 바꾸면 이 화면에도 실시간으로 반영됩니다.

## 교체 알림(🔔)

화면 상단의 **"🔔 알림 받기"** 버튼을 누르면 브라우저 알림 권한을 요청합니다. 허용하면, 이 탭이 열려 있는 동안 어떤 슬롯이 "곧 교체" 또는 "교체 필요" 상태가 될 때 브라우저 알림이 뜨고, 브라우저 탭 제목에도 경고 개수가 표시됩니다.
